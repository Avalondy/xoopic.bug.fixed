...
{  
  ...
  v3Ucutoff.setValue(ostring("0.0"));
	...
	//------------------------------------
  thetaUpperLimit.setNameAndDescription("thetaUpperLimit","Upper theta limit for angular distribution");
  thetaUpperLimit.setValue(ostring("0.0"));
	parameterList.add(&thetaUpperLimit);
	//------------------------------------
...
Boundary* EmitterParams::CreateCounterPart()  //sets up distribution and species
{
  ...
  //------------------------------------
  distribution->setTheta(thetaUpperLimit.getValue());
  //------------------------------------
	return NULL;
}
