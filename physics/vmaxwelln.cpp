...
Vector3 MaxwellianFlux::get_V(const Vector3& wall_normal)
{
	...
	if (thetaUpperLimit > 0 && thetaUpperLimit <= M_PI) AngularDistr(v, thetaUpperLimit);// make v angular distributed
	
	return v;
	
}

void MaxwellianFlux::AngularDistr(Vector3& v, const Scalar& thetaU)
{

  Scalar up1, up2, up3, mag;
  Scalar r11, r12, r13, r21, r22, r23, r31, r32, r33;
	
	//Scalar theta1  = thetaU*frand();
	Scalar theta1  = acos(1 - (1-cos(thetaU))*frand() );
  Scalar costheta = cos(theta1);
  Scalar sintheta = sin(theta1);
	
  Scalar phi1  = TWOPI*frand();
  Scalar cosphi = cos(phi1);
  Scalar sinphi = sin(phi1);
	
	Scalar vmag = v.magnitude();
  r13 = v.e1()/vmag;
  r23 = v.e2()/vmag;
  r33 = v.e3()/vmag;
  ...
  v.set_e1(vmag*(r11*sintheta*cosphi +r12*sintheta*sinphi +r13*costheta));
  v.set_e2(vmag*(r21*sintheta*cosphi +r22*sintheta*sinphi +r23*costheta)); 
  v.set_e3(vmag*(r31*sintheta*cosphi +r32*sintheta*sinphi +r33*costheta));
}
